...
nohup java -Xms1024m -Xmx1024m -XX:MaxMetaspaceSize=256m -Dio.netty.availableProcessors=200 -jar $PWD/karate/karate-1.1.0.jar -p 8085 -m=$PWD/payment-service-mock.feature >> $PWD/nohup.out 2>&1 &

echo "Running pid $(jps -l | grep try-karate-mock)"
echo "$(jps -l | grep try-karate-mock | awk '{print $1}')" > start.pid